...
const AdminNotificationPage = ({navigation, route}) => {
  ...
  const [messageType, setMessageType] = useState('Message'); // Default to PM

  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const response = await fetch(`${API_URL}/api/notification?userId=${route?.params?.userId}`);
  ...
  const filteredNotifications = notifications.filter((item) => {
    const matchesView = viewMode === 'Inbox' ? (item.recipient?.toString() === route?.params?.userId?.toString() || item.recipient?.toString() === 'Admin') : item.sender?.toString() === route?.params?.userId.toString();
  ...
  const renderItem = ({ item }) => {
    const content = (
      <>
        ...
        {(item.type === 'Message' || item.type === 'Announcement') && (
          <>
            <Text>
        ...
        {(item.type === 'License' || item.type === 'Certification' || item.type =='License Expiry' || item.type =='Cancel Request') && (
          <>
            <Text>
              {viewMode === 'Inbox' ? `From: ${item.senderName}` : `To: ${item.recipient_name}`}
            </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <Picker
              ...
              style={styles.filterPicker}
            >
              <Picker.Item label="Private Message" value="Message" />
              <Picker.Item label="Announcement (All Guides)" value="Announcement" />
            </Picker>
